...
with DAG('sparkify-no-partition',
    ...
    load_time_table = GoogleCloudStorageToBigQueryOperator(
        task_id='load-time-table-to-bq',
        bucket="{{dag_run.conf['bucket']}}",
        # Wildcard for objects created by spark job to be written to BigQuery
        # Reads the relative path to the objects transformed by the spark job from the POST message.
        source_objects=["transformed/partitioned/time_table.parquet/part-*"],
        destination_project_dataset_table='egen.time_table_no_partition',
        autodetect=True,
        source_format='PARQUET',  # Note that our spark job does json -> csv conversion.
        create_disposition='CREATE_IF_NEEDED',
        skip_leading_rows=0,
    ...
    delete_transformed_files = BashOperator(
        ...
        bash_command="gsutil -m rm -r gs://{{ dag_run.conf['bucket'] }}/transformed/partitioned")
    ...
    submit_pyspark.set_downstream([delete_cluster, bq_load, load_time_table])

    bq_load.set_downstream(delete_transformed_files)

    move_failed_files.set_upstream([bq_load, submit_pyspark, load_time_table])
